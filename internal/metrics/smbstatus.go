...
// SmbStatusTCon represents a 'tcon' output field
type SmbStatusTCon struct {
...
type SmbStatusJSON struct {
	...
	TCons       map[string]SmbStatusTCon       `json:"tcons"`
...
func RunSmbStatusShares() ([]SmbStatusTCon, error) {
	...
	return []SmbStatusTCon{}, err
}

func parseSmbStatusSharesAsJSON(dat string) ([]SmbStatusTCon, error) {
	tcons := []SmbStatusTCon{}
...
func SmbStatusSharesByMachine() (map[string][]SmbStatusTCon, error) {
	shares, err := RunSmbStatusShares()
	if err != nil {
		return map[string][]SmbStatusTCon{}, err
	}
	return makeSmbSharesMap(shares), nil
}

func makeSmbSharesMap(shares []SmbStatusTCon) map[string][]SmbStatusTCon {
	ret := map[string][]SmbStatusTCon{}
...
// representation.
func parseSmbStatusShares(data string) ([]SmbStatusTCon, error) {
	shares := []SmbStatusTCon{}
	serviceIndex := 0
	pidIndex := 0
	machineIndex := 0
	connectedAtIndex := 0
